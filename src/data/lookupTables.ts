...
/**
 * جداول تصميم الحواجز الجانبية — AASHTO Roadside Design Guide، الطبعة الرابعة 2011
 * (مع تصحيحات 2012 و2015)، القسم 5.
 * المصدر: الجدول 5-7 (خط النفور LS)، الجدول 5-9 (معدلات الفلير)،
 * والجدول 5-10(a) (طول الخروج LR بالوحدات المترية).
 * ملاحظة: LR يعتمد على السرعة التصميمية وحجم المرور اليومي ADT معاً — وليس على السرعة وحدها.
 */

/** فئات حجم المرور اليومي المستخدمة في الجدول 5-10(a) */
export type AdtBand = "over10000" | "5000to10000" | "1000to5000" | "under1000";

export const ADT_BANDS: { key: AdtBand; labelAr: string }[] = [
  { key: "over10000", labelAr: "أكثر من 10,000 مركبة/يوم" },
  { key: "5000to10000", labelAr: "من 5,000 إلى 10,000 مركبة/يوم" },
  { key: "1000to5000", labelAr: "من 1,000 إلى 5,000 مركبة/يوم" },
  { key: "under1000", labelAr: "أقل من 1,000 مركبة/يوم" }
];

/** الجدول 5-10(a): طول الخروج LR بالمتر حسب السرعة وحجم المرور ADT */
export const RUNOUT_LENGTH_TABLE: Record<number, Record<AdtBand, number>> = {
  130: { over10000: 143, "5000to10000": 131, "1000to5000": 116, under1000: 101 },
  110: { over10000: 110, "5000to10000": 101, "1000to5000": 88, under1000: 76 },
  100: { over10000: 91, "5000to10000": 76, "1000to5000": 64, under1000: 61 },
  80: { over10000: 70, "5000to10000": 58, "1000to5000": 49, under1000: 46 },
  60: { over10000: 49, "5000to10000": 40, "1000to5000": 34, under1000: 30 },
  50: { over10000: 34, "5000to10000": 27, "1000to5000": 24, under1000: 21 }
};

/** السرعات المنصوص عليها في الجدول 5-10(a) تصاعدياً */
export const RUNOUT_SPEEDS = [50, 60, 80, 100, 110, 130];

/**
 * يُرجع طول الخروج LR للسرعة وفئة ADT.
 * السرعات غير المدرجة في الجدول تُستوفى خطياً بين أقرب قيمتين،
 * ويُرجع interpolated = true للتنبيه بأن القيمة غير منصوصة حرفياً.
 */
export function getRunoutLength(
  speed: number,
  adt: AdtBand
): { lr: number; interpolated: boolean } {
  if (RUNOUT_LENGTH_TABLE[speed]) {
    return { lr: RUNOUT_LENGTH_TABLE[speed][adt], interpolated: false };
  }
  const speeds = RUNOUT_SPEEDS;
  if (speed <= speeds[0]) return { lr: RUNOUT_LENGTH_TABLE[speeds[0]][adt], interpolated: true };
  const last = speeds[speeds.length - 1];
  if (speed >= last) return { lr: RUNOUT_LENGTH_TABLE[last][adt], interpolated: true };
  let lo = speeds[0];
  let hi = last;
  ...
  const a = RUNOUT_LENGTH_TABLE[lo][adt];
  const b = RUNOUT_LENGTH_TABLE[hi][adt];
  const lr = a + ((b - a) * (speed - lo)) / (hi - lo);
  return { lr: Math.round(lr), interpolated: true };
}

/** الجدول 5-7: إزاحة خط النفور LS بالمتر */
export const SHY_LINE_OFFSET: Record<number, number> = {
  130: 3.7, 120: 3.2, 110: 2.8, 100: 2.4, 90: 2.2,
  80: 2.0, 70: 1.7, 60: 1.4, 50: 1.1
};

/**
 * الجدول 5-9: معدلات الفلير القصوى الموصى بها (a:b — أدخل b).
 * insideShy = الحاجز داخل خط النفور.
 * rigid = حاجز صلب (خرساني) عند/خارج خط النفور.
 * semiRigid = حاجز شبه صلب (معدني W-beam) عند/خارج خط النفور.
 */
export const FLARE_RATE_TABLE: Record<
  number,
  { insideShy: number; rigid: number; semiRigid: number }
> = {
  110: { insideShy: 30, rigid: 20, semiRigid: 15 },
  100: { insideShy: 26, rigid: 18, semiRigid: 14 },
  90: { insideShy: 24, rigid: 16, semiRigid: 12 },
  80: { insideShy: 21, rigid: 14, semiRigid: 11 },
  70: { insideShy: 18, rigid: 12, semiRigid: 10 },
  60: { insideShy: 16, rigid: 10, semiRigid: 8 },
  50: { insideShy: 13, rigid: 8, semiRigid: 7 }
};

export const FLARE_SPEEDS = [50, 60, 70, 80, 90, 100, 110];

/**
 * يُرجع معدل الفلير المناسب حسب الجدول 5-9.
 * إذا كانت إزاحة الحاجز L2 أقل من خط النفور LS فالحاجز داخل خط النفور
 * ويُستخدم عمود insideShy، وإلا يُستخدم العمود حسب نوع الحاجز.
 * للسرعات فوق 110 تُستخدم قيم صف 110 (أعلى صف منصوص).
 */
export function getFlareRate(
  speed: number,
  barrierType: "steel" | "concrete",
  l2: number,
  ls: number
): { rate: number; column: "insideShy" | "rigid" | "semiRigid"; labelAr: string } {
  const speeds = FLARE_SPEEDS;
  let key = speeds[0];
  for (const s of speeds) if (speed >= s) key = s;
  const row = FLARE_RATE_TABLE[key];
  if (l2 < ls) {
    return { rate: row.insideShy, column: "insideShy", labelAr: "داخل خط النفور" };
  }
  if (barrierType === "concrete") {
    return { rate: row.rigid, column: "rigid", labelAr: "حاجز صلب عند/خارج خط النفور" };
  }
  return { rate: row.semiRigid, column: "semiRigid", labelAr: "حاجز شبه صلب عند/خارج خط النفور" };
}

/** يُرجع قيمة LS للسرعة (تُقرّب لأقرب سرعة منصوصة نزولاً) */
export function getShyLine(speed: number): number {
  const speeds = Object.keys(SHY_LINE_OFFSET).map(Number).sort((a, b) => a - b);
  let key = speeds[0];
  for (const s of speeds) if (speed >= s) key = s;
  return SHY_LINE_OFFSET[key];
}
...
/**
 * المنطقة الخالية وفق نهج النظام الآمن — كود الطرق السعودي 304،
 * الجدول 4-2، ص14 (المصدر: TRB 2015).
 * القيم مسافات منطقة خالية بالمتر وليست مسافات إدراك واستجابة.
 * prt12 = عند زمن إدراك ورد فعل 1.2 ثانية، prt25 = عند 2.5 ثانية.
 */
...
export const REFERENCES_DATA: ReferenceItem[] = [
  {
    ...
    nameEn: "Saudi Highway Code 301 (Geometric Design)",
    code: "SHC-301",
    type: "رئيسي وعام",
  ...
  {
    id: "src304",
    nameAr: "كود الطرق السعودي 304 (تصميم مرافق الطرق ومنافعها — أنظمة السلامة الخاملة)",
    nameEn: "Saudi Highway Code 304 (Passive Safety Systems)",
    code: "SHC-304",
    type: "سلامة جوانب الطرق",
    notes: "أنظمة تقييد المركبات وحواجز السلامة والنهايات، وتصنيف المخاطر إلى أربع فئات، والمسافات الحرجة A و AE، والمنطقة الخالية وفق نهج النظام الآمن (جدول 4-2)، ومستويات الاحتواء وفق SASO EN 1317.",
    fileName: "304 AR.pdf"
  },
  {
    id: "src305",
    nameAr: "كود الطرق السعودي 305 (تصميم منطقة أعمال الطرق)",
    nameEn: "Saudi Highway Code 305 (Work Zone Design)",
    code: "SHC-305",
    type: "مناطق أعمال الطرق",
    notes: "تصميم وتنظيم مناطق أعمال الطرق: اللافتات المؤقتة، التوجيه القنواتي، أطوال الاستدقاق، المناطق العازلة، التحويلات، وأجهزة التحكم المروري، والمخططات النموذجية (الملحق ب).",
    fileName: "305_compressed.pdf"
  },
  {
    id: "src401",
    nameAr: "كود الطرق السعودي 401 (التحكم المروري)",
    nameEn: "Saudi Highway Code 401 (Traffic Control)",
    code: "SHC-401",
    type: "هندسة المرور",
    notes: "أنظمة الإشارات الضوئية والتحكم المروري للتقاطعات والطرق الحضرية.",
    fileName: "401 AR.pdf"
  },
  {
    id: "src602",
    nameAr: "كود الطرق السعودي 602 (الدليل الموحد لأجهزة التحكم المروري)",
    nameEn: "Saudi Highway Code 602 (Unified Manual of Traffic Control Devices)",
    code: "SHC-602",
    type: "أجهزة التحكم المروري",
    notes: "نظير MUTCD السعودي — الباب الثالث اللافتات، الباب الرابع الخطوط الأرضية والدهانات وعيون القطط، الباب الخامس الإشارات الضوئية، الباب السادس مناطق المدارس.",
    fileName: "602 AR.pdf"
  },
  {
    id: "src603",
    nameAr: "كود الطرق السعودي 603 (سلامة الطرق)",
    nameEn: "Saudi Highway Code 603 (Road Safety)",
    code: "SHC-603",
    type: "سلامة الطرق",
    notes: "منهجيات سلامة الطرق: تحليل بيانات الحوادث، إدارة السلامة وفحص الشبكة، تدقيق سلامة الطرق، النماذج التنبؤية SPFs وCMFs، إدارة السرعة وتهدئة المرور، وسلامة الأنفاق.",
    fileName: "603_AR.pdf"
  },
  {
    id: "aashto_green",
  ...
  {
    id: "aashto_rdg",
    nameAr: "دليل آشتو لتصميم جوانب الطرق — الطبعة الرابعة 2011 (مع تصحيحات 2012 و2015)",
    nameEn: "AASHTO Roadside Design Guide, 4th Edition (2011, with 2012 & 2015 Errata)",
    code: "AASHTO RDG",
    type: "مرجع دولي",
    notes: "المرجع المعتمد لحاسبات الحواجز والمنطقة الخالية: طول الحاجة (Length of Need)، الجداول 5-7 و5-9 و5-10(a)، النهايات ومخففات الصدمات، وحواجز الجزيرة الوسطية. نسخة كاملة بطبقة نصية قابلة للبحث.",
    fileName: "AASHTO-RSDG-4-2011-with-2012--2015-errata.pdf"
  },
  {
    id: "mutcd11",
